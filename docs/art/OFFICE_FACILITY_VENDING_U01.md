...
owner approval on 2026-07-29. Water W01 subsequently received its independent
F8 approval, so isolated Coffee C01 production is now unlocked. Coffee still
requires its own gates. F9 room placement and F10 runtime integration remain
...
npm run art:facility:vending:u01:rebuild:check
npm run art:facility:vending:u01:check
```

The rebuild check compares deterministic generated bytes on the producer
workstation. The portable CI check performs contract validation, source and
dependency hash checks, exact file-set checks, geometry and viewport checks,
all 108 attachment cases, reservation samples, rejected orientations, and
Active Office isolation without requiring Pillow or platform fonts.

## F8 owner decision

The owner reviewed all twelve U01-r03 boards together and approved this exact
hash set on 2026-07-29, including:
...
inherit U01 approval. Water W01 received its own F8 approval on 2026-07-29,
unlocking isolated Coffee C01 production. Coffee approval, F9, F10, and Active
Office imports remain blocked.
