...
`npm run art:workstation:step5:r05:r02:check` is the current portable
workstation authority gate. It validates the owner-approved 32-pixel coordinate
system, all 216 character seat sockets, the 64-pixel depthwise desk join,
monitor/keyboard physical layer order, isolated pair proof, browser evidence,
and byte-identical Active Office baseline. Regenerate the deterministic
manifest and boards locally with:

```bash
npm run art:workstation:step5:r05:r02
npm run art:workstation:step5:r05:r02:check
```

R03, R04, and R05 final checks below preserve historical or rejected evidence;
none of them grants current assembly authority.
...
The current owner-approved isolated lab is
`/?lab=office-workstation-r05`. R05-r02 browser captures are produced from the
real lab and are not regenerated by the Python command. The older
`/?lab=office-workstation-v3-step5` route is historical R04 evidence.
