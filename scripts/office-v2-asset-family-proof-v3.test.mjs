...
import { buildProofFamily } from "./office-v2-asset-family-proof-v3.mjs";

function tempRoot() {
  return mkdtempSync(join(tmpdir(), "office-v2-family-v3-"));
}

test("builds selected concept 3 as a spec-only modular workstation family", () => {
  const root = tempRoot();
  try {
    const result = buildProofFamily({
      outputRootA: join(root, "build-a"),
      outputRootB: join(root, "build-b"),
      reportRoot: join(root, "reports"),
    });
    assert.deepEqual(result.report.supportedMasks, [0, 2, 8, 10]);
    assert.equal(result.report.taskId, "P5-W6.5-R1-V3");
    assert.equal(result.report.selectedConcept, 3);
    assert.equal(result.report.admission, "spec-only");
    assert.equal(result.report.review.state, "pending-owner-review");
    assert.equal(result.report.visualContract.status, "selected-concept-frozen");
    assert.deepEqual(result.report.workstationDock.spritePx, { x: 56, y: 56 });
    assert.equal(result.report.workstationDock.fullSeatedIntegration, "deferred");
    assert.deepEqual(result.report.seamComposition, {
      order: ["mask-2", "mask-10", "mask-8"],
      translationPx: { x: 64, y: 32 },
    });
    ...
    assert.equal(result.evidence.some(({ kind }) => kind === "workstation-dock-diagnostic"), true);
    assert.equal(result.evidence.some(({ kind }) => kind === "seated-actor-contact-overlay"), false);
    assert.equal(result.evidence.some(({ kind }) => kind === "fit-analysis"), true);
    for (const frame of result.source.frames) {
      assert.deepEqual(frame.contacts, [{ id: "workstation-dock", xPx: 56, yPx: 56, requiredAlpha: "any", role: "chair-dock" }]);
    }
    const native = decodePng(readFileSync(join(root, "reports/review/enlarged-native-scale/mask-0.png")));
    assert.deepEqual({ widthPx: native.widthPx, heightPx: native.heightPx }, { widthPx: 704, heightPx: 384 });
...
test("changing the selected concept accent changes deterministic V3 output evidence", () => {
  const first = tempRoot();
  const second = tempRoot();
  try {
    const firstResult = buildProofFamily({
      outputRootA: join(first, "a"),
      outputRootB: join(first, "b"),
      reportRoot: join(first, "reports"),
    });
    const changedSource = JSON.parse(readFileSync("assets/office-v2/sources/workstation-basic/v3/source.json", "utf8"));
    changedSource.frames[0].privacyAccent = [116, 72, 164, 255];
    const changedSourcePath = join(second, "source.json");
    writeFileSync(changedSourcePath, JSON.stringify(changedSource));
    const secondResult = buildProofFamily({
      sourcePath: changedSourcePath,
      outputRootA: join(second, "a"),
      outputRootB: join(second, "b"),
      reportRoot: join(second, "reports"),
    });
