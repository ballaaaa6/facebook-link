...
test("uses the completed semantic-grid v6 background and aligned overlays", () => {
  assert.match(runtimeSource, /office-c-background-modern-v8-current\.png/);
  ...
  assert.match(runtimeSource, /whiteboard: \{ x: 117, y: 118, width: 350, height: 235 \}/);
  assert.match(runtimeSource, /whiteboardContent: \{ x: 156, y: 157, width: 272, height: 157 \}/);
});
