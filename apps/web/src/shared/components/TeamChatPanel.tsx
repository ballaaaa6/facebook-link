...
export function TeamChatPanel() {
  const brain = useMemo(() => createBrainProvider("mock"), []);
  const [agentId, setAgentId] = useState("");
  const [draft, setDraft] = useState("");
  const [busy, setBusy] = useState(false);
  const [lines, setLines] = useState<ChatLine[]>([{ id: "welcome", author: "Nova", content: "TeamBrain mock is online. Ask about products, metrics, content, links, or publishing." }]);
  ...
  async function submit(event: FormEvent) {
    ...
    const response = await brain.respond({ workspaceId: "pilot-workspace", conversationId: "browser-demo", ...(agentId ? { selectedAgentId: agentId } : {}), message, recentMessages });
  ...
  return (
    <section className="team-chat card-surface">
      <div className="section-heading"><div><span className="eyebrow">TeamBrain</span><h2>Command the team</h2></div><span className="mock-badge">Mock / safe</span></div>
